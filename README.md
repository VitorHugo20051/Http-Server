Este é um modelo de **README.md** profissional e estruturado, focado na arquitetura técnica que implementaste nos teus ficheiros C.
...
* **Parsing de Protocolo HTTP:** Motor de processamento manual de pedidos HTTP, capaz de extrair *Request Lines*, *Headers* e o corpo das mensagens (incluindo suporte para `x-www-form-urlencoded`).
...
* **`Server.c`**: Lógica de baixo nível para criação e configuração de sockets TCP/IP (`socket`, `bind`, `listen`).
