...
├── HttpServer.c           # Orquestrador principal: ciclo de vida das threads e despacho de rotas
├── HttpRequest.c          # Parser: transforma strings de rede em estruturas de dados organizadas
├── ThreadPool.c           # Gestor de concorrência com conjunto ativo de threads
└── DataStructures/        # Implementações manuais de Listas Ligadas, Filas, Árvores Binárias e Dicionários
```
...
O projeto inclui um `Makefile` para automatizar a compilação de todos os módulos e estruturas de dados.
...
Utiliza o `gcc` com as flags `-Wall -Wextra -g` e gera o executável `server`.
...
O servidor permite registar rotas de forma simples no ficheiro `main.c`:
